...
abstr_fw(Conc,Abstr) :-
  ...
  collect_partition(PartitionL1),
    ...
    ConcIniL=[ConcIni],
    partition_mod_n(11,ConcIni,ConcQN,ConcTrL,ModClsL),
...
refine_abstraction(CritConcState,NewPred) :-
  ...
  retract(fin_lang_up_to(OldLim)), %% !!! Switch off retract(pred_lang(PredLangL)) %% Choose this or ...
    NewLim is OldLim+11, %% Choose this or ...
...
:- fsa_regex_compile(file(trans),Trs),assertz(tr(Trs)).
...
:- sort([0,1],S),assertz(sigma(S)).
:- assertz(fin_lang_up_to(11)).
%%%%%%%%%%%%%%%%%%%%%%%%
