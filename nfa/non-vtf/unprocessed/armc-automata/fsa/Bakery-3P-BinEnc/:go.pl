...
do_one_run(N,AbsReach,Result,Value) :-
 ...
 %%% XXX
 name(N,NLc),atom_chars(PNc,NLc),atom_concat('xxa',PNc,PPc),fsa_write_file(PPc,NewReach),
  ...
  ( (empty(ImmReachBad), !,do_one_run_ph2(NN,AbsReach,NewReach,Result,Value));
    (Result=bad,Value=[ImmReachBad,ImmReachBad,NewReach],write('XXXXXXXXX'),nl, trace) ).
...
do_one_run_ph2(N,AbsReach,NewReach,Result,Value) :-
 ...
 %%% XXX
 name(N,NLa),atom_chars(PNa,NLa),atom_concat('xxb',PNa,PPa),fsa_write_file(PPa,NewAbsReach),
 trace,
...
do_one_run_ph3(N,NewReach,NewAbsReach,Result,Value) :-
 ...
 %%% XXX
 name(N,NLa),atom_chars(PNa,NLa),atom_concat('xxc',PNa,PPa),fsa_write_file(PPa,NewAbsReach),
 trace,
...
one_run(Result,Value) :-
 ...
 %%% XXX
 fsa_write_file(absinit,AbsInit),trace,
...
:- sort(['000','001','010','011','100','101','110','111',c,i,w],S),assertz(sigma(S)).
%%%%%%%%%%%%%%%%%%%%%%%%
