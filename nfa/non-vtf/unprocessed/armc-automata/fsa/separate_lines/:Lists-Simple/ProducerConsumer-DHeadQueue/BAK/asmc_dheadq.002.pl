...
augment_reachable(Line,[[Line,ReachAtLine]|ReachedL],NewSt,[[Line,ReachAtLine3]|ReachedL]) :-
  fsa_regex_compile(min_mh_mb(union(fa(ReachAtLine),fa(NewSt))),ReachAtLine2),
  #ifndef AbsAt
    abstract(ReachAtLine2,ReachAtLine3),
  #else
    ( (member(Line,AbsAt), !, abstract(ReachAtLine2,ReachAtLine3));
      (ReachAtLine2=ReachAtLine3) ),
  #endif
  #ifdef CompAtmSizes
    ReachAtLine3=fa(_,NS1,_,_,TrL1,_),
    length(TrL1,NT1),
    stat_update_max(NS1,NT1),
  #endif
  !.
