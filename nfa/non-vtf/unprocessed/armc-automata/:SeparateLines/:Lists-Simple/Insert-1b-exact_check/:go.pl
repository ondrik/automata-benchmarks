...
do_accel_unwound_loops(St,StN,UnwoundTimes,[/,'_old'],LoopLen,TrL,VeryNewStN,VeryNewTrL,1) :-
  successor_via_ntimes_a_word(UnwoundTimes,St,[/,'_old'],TrL,[St],[C,B,A|_]),
  not(successor_via_a_word(C,[/,'_old'],TrL,C)),
  NextSt is St+1,
  do_accel_unwound_loops(NextSt,StN,UnwoundTimes,[/,'_old'],LoopLen,TrL,NewStN1,NewTrL1,_),
  write('!!! Adding a loop from:'),write(A),write('-'),write(B),write(-),write(C),nl,
  NewStN2 is NewStN1+1,
  add_input_to_new_loop(A,NewStN2,NewStN1,[/,'_old'],NewTrL2),
  NewStN3 is NewStN1+LoopLen,
  add_accel_unwound_loop(NewStN3,NewStN1,[/,'_old'],NewTrL3),
...
do_accel_unwound_loops(St,StN,UnwoundTimes,[/,'_old'],LoopLen,TrL,NewStN,NewTrL,AccDone) :-
  NextSt is St+1,
  do_accel_unwound_loops(NextSt,StN,UnwoundTimes,[/,'_old'],LoopLen,TrL,NewStN,NewTrL,AccDone).
...
accel_unwound_loops([/,'_old'],States,AccelStates) :-
  ...
  write('Accelerating via '),write([/,'_old']),write(' loops unwound '),
    write(UnwoundTimes),write(' times and added to new states...'),nl,
  do_accel_unwound_loops(0,StN,UnwoundTimes,[/,'_old'],LoopLen,TrL,NewStN,NewTrL,AccDone),
  ( (AccDone=0, !, AccelStates=States);
    (fsa_regex_compile(mb(cleanup(fa(fa(PredModule,NewStN,IniL,FinL,NewTrL,[])))),AccelStates)) ).
...
do_accel_several_unwound_loops([[/,'_old']|LoopsRestL],States,AccelStates) :-
  ...
  write('Accelerating via '),write([/,'_old']),write(' loops unwound '),
    ...
    do_accel_unwound_loops(0,StN,UnwoundTimes,[/,'_old'],LoopLen,TrL,NewStN,NewTrL,_),
...
all_pos_loops_gen_loop(0,_,_,_,[/,'_old'],[[/,'_old']]) :- !.

all_pos_loops_gen_loop(N,[DtSym|DtSymL],AllDtSymL,SepSym,[/,'_old'],LoopsL3) :-
  N>0,
  NN is N-1,
  all_pos_loops_gen_loop(NN,AllDtSymL,AllDtSymL,SepSym,[DtSym,SepSym|[/,'_old']],LoopsL1),
  all_pos_loops_gen_loop(N,DtSymL,AllDtSymL,SepSym,[/,'_old'],LoopsL2),
