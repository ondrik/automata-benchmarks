...
is_included(A,B) :-
  ...
  empty(E),
  fsa_regex_compile(cleanup(intersect(fa(A),complement(fa(B)))),E),
  !.
...
abstr_fw(Conc,Abstr) :-
  ...
  fsa_regex_compile(cleanup(fa(Abstr1)),Abstr).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fst_to_minfsa(fa(t(fsa_frozen,fsa_frozen),QN,IL,FL,TrL,JL),FA2) :-
  ...
  fsa_regex_compile(cleanup(fa(FA1)),FA2).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Saving a non-determinized FA in the FSA format
% (Str is a string to put into the file name.)
% save_nfa(A,Str) :-
% ((retract(nfaNo(N)),!,NN is N+1,assertz(nfaNo(NN)));
% (N=0,assertz(nfaNo(1)))),
% name(N,NL),atom_chars(PN,NL),
% atom_concat('armcNFA_',Str,PP1),
% atom_concat(PP1,'_',PP2),
% atom_concat(PP2,PN,PP),
% fsa_write_file(PP,A),
% !.
%-------------------------------------------------------------------------------
do_save_sigma([A|SL],N) :-
  name(N,NL),atom_chars(PN,NL),atom_concat('a',PN,PP),
  write(PP),write(':1 '),
  assertz(sigma_translated(A,PP)),
  NN is N+1,
  do_save_sigma(SL,NN).
do_save_sigma([],_) :- write('x:0'). % The 'initial' letter.
save_sigma :-
  write('Ops '),
  sigma(SL),
  do_save_sigma(SL,0),
  ...
  !.
%...............................................................................
do_save_states(Q,Q) :- nl, !.
do_save_states(Q,QN) :-
  write(q),
  write(Q),
  write(' '),
  QQ is Q+1,
  do_save_states(QQ,QN).
save_states(QN) :-
  write('States '),
  do_save_states(0,QN),
  !.
...
save_init([Q|IL]) :-
  ...
  save_init(IL).
%...............................................................................
save_single_trans(Q1,A,Q2) :-
  sigma_translated(A,TransA),
  write(TransA),write('(q'),write(Q1),write(') -> q'),write(Q2),
  nl.
%...............................................................................
do_save_trans(Q1,[],Q2) :- !.
do_save_trans(Q1,[A|AL],Q2) :-
  save_single_trans(Q1,A,Q2),
  do_save_trans(Q1,AL,Q2).
%...............................................................................
save_trans([]) :- !.
save_trans([trans(Q1,in(AL),Q2)|TrL]) :-
  !,
  do_save_trans(Q1,AL,Q2),
  save_trans(TrL).
save_trans([trans(Q1,A,Q2)|TrL]) :-
  atom(A),
  !,
  save_single_trans(Q1,A,Q2),
  save_trans(TrL).
save_trans([Tr|_]) :-
  write('>>> '),
  write(Tr),
  write('found on a transition: NOT SUPPORTED in save_trans yet!'), nl,
  halt.
%...............................................................................
% Saving a non-determinized FA in the Timbuk format
% (Str is a string to put into the file name.)
save_nfa(fa(_,QN,IL,FL,TrL,[]),Str) :-
  ((retract(nfaNo(N)),!,NN is N+1,assertz(nfaNo(NN)));
   (N=0,assertz(nfaNo(1)))),
  name(N,NL),atom_chars(PN,NL),
    atom_concat('armcNFA_',Str,PP1),
    atom_concat(PP1,'_',PP2),
    atom_concat(PP2,PN,PP),
  open(PP,write,Stream),
  tell(Stream),
  save_sigma, nl,
  write('Automaton A'), nl,
  save_states(QN),
  save_final(FL),
  ...
  told,
  !.
save_nfa(fa(Pr,QN,IL,FL,TrL,[J|JL]),Str) :-
  fsa_regex_compile(efree(fa(fa(Pr,QN,IL,FL,TrL,[J|JL]))),A),
  save_nfa(A,Str),
  !.
%-------------------------------------------------------------------------------
do_save_tr_states(Q,Q) :- !.
do_save_tr_states(Q,QN) :-
  write('; "q'),
  write(Q),
  write('"'),
  QQ is Q+1,
  do_save_tr_states(QQ,QN).
save_tr_states(QN) :-
  write('["q0"'),
  do_save_tr_states(1,QN),
  write(']'),
  !.
%...............................................................................
do_save_tr_final([]) :- !.
do_save_tr_final([Q|FL]) :-
  write('; "q'),
  write(Q),
  write('"'),
  do_save_tr_final(FL).
...
save_tr_init([Q|IL]) :-
  ...
  do_save_tr_init(IL).
%...............................................................................
save_single_trans_tr(From,To,In,Out) :-
  write(';'),
  nl,
  write('  ("'),
  write(In),
  ...
  write('")').
%...............................................................................
save_id_trans_tr(_,_,[]) :- !.
save_id_trans_tr(From,To,[InOut|InOutL]) :-
  save_single_trans_tr(From,To,InOut,InOut),
  save_id_trans_tr(From,To,InOutL).
%...............................................................................
save_inl_trans_tr(_,_,[],_) :- !.
save_inl_trans_tr(From,To,[In|InL],Out) :-
  save_single_trans_tr(From,To,In,Out),
  save_inl_trans_tr(From,To,InL,Out),
  !.
%...............................................................................
save_inl_outl_trans_tr(_,_,_,[]) :- !.
save_inl_outl_trans_tr(From,To,InL,[Out|OutL]) :-
  save_inl_trans_tr(From,To,InL,Out),
  save_inl_outl_trans_tr(From,To,InL,OutL),
  !.
%...............................................................................
save_trans_compl_tr([],_,[]) :- !.
save_trans_compl_tr([A|L],NotL,RestL) :-
  member(A,NotL),
  !,
  save_trans_compl_tr(L,NotL,RestL).
save_trans_compl_tr([A|L],NotL,[A|RestL]) :-
  save_trans_compl_tr(L,NotL,RestL).
%...............................................................................
save_trans_tr([]) :- !.
save_trans_tr([trans(From,$@(in(InOutL)) / $@(in(InOutL)),To)|TrL]) :-
  save_id_trans_tr(From,To,InOutL),
  save_trans_tr(TrL),
  !.
save_trans_tr([trans(From,$@(not_in(NotL)) / $@(not_in(NotL)),To)|TrL]) :-
  sigma(SigmaL),
  save_trans_compl_tr(SigmaL,NotL,ComplL),
  save_id_trans_tr(From,To,ComplL),
  save_trans_tr(TrL),
  !.
save_trans_tr([trans(From,PredIn / PredOut,To)|TrL]) :-
  ( (PredIn=in(InL),!);
    (PredIn=not_in(NotInL),!,sigma(SigmaL),save_trans_compl_tr(SigmaL,NotInL,InL));
    (PredIn=In,!,InL=[In]) ),
  ( (PredOut=in(OutL),!);
    (PredOut=not_in(NotOutL),!,sigma(SigmaL),save_trans_compl_tr(SigmaL,NotOutL,OutL));
    (PredOut=Out,!,OutL=[Out]) ),
  save_inl_outl_trans_tr(From,To,InL,OutL),
  save_trans_tr(TrL),
  !.
%...............................................................................
% Saving an FSA transducer into the ARTMC format used together with Timbuk
% (Str is a string to put into the file name.)
save_tr(fa(t(_,_),QN,IL,FL,TrL,[]),Str) :-
  ((retract(nfaNo(N)),!,NN is N+1,assertz(nfaNo(NN)));
   (N=0,assertz(nfaNo(1)))),
  name(N,NL),atom_chars(PN,NL),
    atom_concat('armcTransd_',Str,PP1),
    atom_concat(PP1,'_',PP2),
    atom_concat(PP2,PN,PP),
  open(PP,write,Stream),
  tell(Stream),
...
fire_n_tr([Tr|TrL],From,UnionTo) :-
  ...
  fsa_regex_compile(cleanup(range(compose(fa(From),fa(Tr)))),To),
  fsa_regex_compile(cleanup(union(fa(To),fa(AuxUnionTo))),UnionTo),
  !.
fire_n_tr([],From,From).
%...............................................................................
...
do_one_run(N,AbsReach,Result,Value) :-
  ...
  write('Abstract source automaton: '),write(NS1),write(','),write(NT1),nl,
      fsa_regex_compile(cleanup(range(compose(fa(AbsReach),fa(TR)))),NewReach),
...
%% AbsReach=fa(_,NS1,_,_,T1,_),length(T1,NT1),
%% write('Abstract source automaton: '),write(NS1),write(','),write(NT1),nl,
%% fsa_regex_compile(cleanup(range(compose(fa(AbsReach),fa(TR)))),OneStepReach),
  ...
  ( (SubResult=good, !,Result=SubResult,Value=SubValue);
    ...
    (itr(ITR),
     ...
     write('Computing pre...'),nl,
        fsa_regex_compile(cleanup(range(compose(fa(NextReachBad1),fa(ITR)))),SrcBad),
...
add_self_loops(fa(X,S,I,F,T1,Y),A) :-
  ...
  A=fa(X,S,I,F,T2,Y).
%% fsa_regex_compile(cleanup(fa(fa(X,S,I,F,T2,Y))),A).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%===============================================================================
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
lrn_do_nlenpr_reachable_upto_n(From,AllUpToN,TR,Reachable) :-
  %%% write('>> One step on... '), nl,
  fsa_regex_compile(cleanup(range(compose(fa(From),fa(TR)))),TR_From),
...
lrn_try_collapse(Reach,Init,Bad,TR,AbsReach) :-
  ...
  %% fsa_regex_compile(cleanup(cleanup(range(compose(fa(AbsReach),fa(TR))))),AbsReach), %% Choose this or ...
  %% fsa_regex_compile(cleanup(range(compose(fa(AbsReach),fa(TR)))),AbsReach), %% Choose this or ^^^
...
tb_lang_of_state(N,fa(Pred,NQ,_,FinL,TL,JL),A) :-
  fsa_regex_compile(cleanup(fa(fa(Pred,NQ,[N],FinL,TL,JL))),A).
%-------------------------------------------------------------------------------
...
% fsa_regex_compile(cleanup(fa(fa(Pred,NQ,[0],FinL,NewTrl,JL))),NewAut),
...
abs_tracht_barzd(fa(Pred,NQ,[0],FinL,TrL,JL),NewAut) :-
  ...
  %% fsa_regex_compile(cleanup(fa(fa(Pred,NQ,[0],FinL,NewTrl,JL))),NewAut), %% Choose this or vvv
...
accel_unwound_self_loops(LoopLetter,States,AccelStates) :-
  ...
  ( (AccDone=0, !, AccelStates=States);
      (fsa_regex_compile(cleanup(cleanup(fa(fa(PredModule,StN,IniL,FinL,NewTrL,[])))),AccelStates)) ).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The main loop of accelerating via unwound self-loops
...
accel_unwound_loops(LoopL,States,AccelStates) :-
  ...
  ( (AccDone=0, !, AccelStates=States);
      (fsa_regex_compile(cleanup(cleanup(fa(fa(PredModule,NewStN,IniL,FinL,NewTrL,[])))),AccelStates)) ).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The main loop of accelerating via unwound general loops
% !!! Adding a special state with the new self-loop !!!
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
accel_several_unwound_loops(LoopsL,States,MinAccelStates) :-
  do_accel_several_unwound_loops(LoopsL,States,AccelStates),
    fsa_regex_compile(cleanup(cleanup(fa(AccelStates))),MinAccelStates).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
septr_do_one_run_ph1(N,FromSt,ToLine,Tr,ITr,TrL,ReachedL,Result,Value) :-
  ...
  write('Source automaton: '),write(NS1),write(','),write(NT1),nl,
    fsa_regex_compile(cleanup(range(compose(fa(FromSt),fa(Tr)))),ToSt),
...
augment_reachable(Line,[[Line,ReachAtLine]|ReachedL],NewSt,[[Line,ReachAtLine2]|ReachedL]) :-
  fsa_regex_compile(cleanup(union(fa(ReachAtLine),fa(NewSt))),ReachAtLine2),
  !.
...
septr_do_one_run_ph5(N,FromSt,ToLine,ToSt,ToAbSt,Tr,ITr,TrL,ReachedL,[ToErr,ToBackErr],Result,Value) :-
  %% write('... phase 5'),nl,
  write('Computing pre...'),nl,
    fsa_regex_compile(cleanup(range(compose(fa(ToErr),fa(ITr)))),BackErr),
...
do_transd_to_lenpr(Transd1,Sigma,Transd2) :-
  ...
  sort(TrL4,TrL5),
  fsa_regex_compile(cleanup(fa(fa(t(fsa_preds,fsa_preds),NQ,IniL,FinL,TrL5,[]))),Transd2),
  !.
%----------------------------------------------------------------------------------
...
do_atm_to_lenpr(Atm1,Sigma,Atm2) :-
  ...
  sort(TrL3,TrL4),
  fsa_regex_compile(cleanup(fa(fa(r(fsa_preds),NQ,IniL,FinL,TrL4,[]))),Atm2),
  !.
%----------------------------------------------------------------------------------
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%
%% :- fsa_global_set(hash_size,10000000).
...
%% :- fsa_regex_compile(file(tr),TR),assertz(tr(TR)).
%%......................
