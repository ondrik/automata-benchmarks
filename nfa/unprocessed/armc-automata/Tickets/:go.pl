...
abstr_fw(Conc,Abstr) :-
  ...
  Conc=fa(ConcSym,ConcQN,ConcIniL,ConcFinL,ConcTrL,_),
    ...
    ConcIniL=[ConcIni],
    ini_partition_mod_n(8,ConcIni,ConcQN,ConcTrL,IniClsL),
    ini_fw_classes_mod_n(IniClsL),
...
fl_comp_partition(N,QN,FL,TrL,PartL) :-
  sigma(SigL),
    ...
    ini_partition_mod_n(8,0,QN,TrL,IniClsL1),
...
ft_comp_partition(N,QN,TrL,PartL) :-
  sigma(SigL),
    ...
    ini_partition_mod_n(8,0,QN,TrL,IniClsL),
...
abstr_fl_refby_ft(Conc,Abstr) :-
  ...
  sigma(SigL),
    ...
    ini_partition_mod_n(8,0,ConcQN,ConcTrL,IniClsL1),
    split_ini_part_mod_n_fin_nonfin(IniClsL1,FL,_,IniClsL),
...
refine_abstraction(CritConcState,NewPred) :-
  ...
  retract(fin_lang_up_to(OldLim)), %% !!! Switch off retract(pred_lang(PredLangL)) %% Choose this or ...
    NewLim is OldLim+8, %% Choose this or ...
...
accel_unwound_self_loops(LoopLetter,States,AccelStates) :-
  ...
  ( (AccDone=0, !, AccelStates=States);
    (fsa_regex_compile(mb(cleanup(fa(fa(PredModule,StN,IniL,FinL,NewTrL,[])))),AccelStates)) ).
...
do_transd_to_lenpr(Transd1,Sigma,Transd2) :-
  ...
  sort(TrL4,TrL5),
  fsa_regex_compile(mb(fa(fa(t(fsa_preds,fsa_preds),NQ,IniL,FinL,TrL5,[]))),Transd2),
  !.
...
do_atm_to_lenpr(Atm1,Sigma,Atm2) :-
  ...
  sort(TrL3,TrL4),
  fsa_regex_compile(mb(fa(fa(r(fsa_preds),NQ,IniL,FinL,TrL4,[]))),Atm2),
  !.
