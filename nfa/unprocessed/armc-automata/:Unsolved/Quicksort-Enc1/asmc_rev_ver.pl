...
:- fsa_regex_compile(file(initX),Init),assertz(bad(Init)).

%% :- fsa_regex_compile(file(init2),Init),assertz(bad(Init)).

%% :- fsa_regex_compile(file(initX2),Init),assertz(bad(Init)).
...
:- fsa_regex_compile(file('bad-loop'),BAD),assertz(init(BAD)).
