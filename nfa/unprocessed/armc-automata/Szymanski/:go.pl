...
do_one_run(N,AbsReach,Result,Value) :-
  ...
  fsa_regex_compile(minimize(range(compose(fa(AbsReach),fa(TR)))),NewReach),
...
lrn_one_step_on(From,TR,To) :-
    ...
    fsa_regex_compile(minimize(range(compose(fa(From),fa(TR)))),To),
...
lrn_do_nlenpr_reachable_upto_n(From,AllUpToN,TR,Reachable) :-
  %%% write('>> One step on... '), nl,
  fsa_regex_compile(minimize(range(compose(fa(From),fa(TR)))),TR_From),
...
lrn_try_collapse(Reach,Init,Bad,TR,AbsReach) :-
  ...
  %% fsa_regex_compile(minimize(minimize(range(compose(fa(AbsReach),fa(TR))))),AbsReach), %% Choose this or ...
  %% fsa_regex_compile(minimize(range(compose(fa(AbsReach),fa(TR)))),AbsReach), %% Choose this or ^^^
...
tb_lang_of_state(N,fa(Pred,NQ,_,FinL,TL,JL),A) :-
  fsa_regex_compile(minimize(fa(fa(Pred,NQ,[N],FinL,TL,JL))),A).
...
% fsa_regex_compile(minimize(fa(fa(Pred,NQ,[0],FinL,NewTrl,JL))),NewAut),
...
abs_tracht_barzd(fa(Pred,NQ,[0],FinL,TrL,JL),NewAut) :-
  ...
  %% fsa_regex_compile(minimize(fa(fa(Pred,NQ,[0],FinL,NewTrl,JL))),NewAut), %% Choose this or vvv
...
accel_unwound_self_loops(LoopLetter,States,AccelStates) :-
  ...
  ( (AccDone=0, !, AccelStates=States);
    (fsa_regex_compile(minimize(cleanup(fa(fa(PredModule,StN,IniL,FinL,NewTrL,[])))),AccelStates)) ).
...
accel_unwound_loops(LoopL,States,AccelStates) :-
  ...
  ( (AccDone=0, !, AccelStates=States);
    (fsa_regex_compile(minimize(cleanup(fa(fa(PredModule,NewStN,IniL,FinL,NewTrL,[])))),AccelStates)) ).
...
accel_several_unwound_loops(LoopsL,States,MinAccelStates) :-
  ...
  fsa_regex_compile(minimize(cleanup(fa(AccelStates))),MinAccelStates).
...
septr_do_one_run_ph1(N,FromSt,ToLine,Tr,ITr,TrL,ReachedL,Result,Value) :-
  ...
  fsa_regex_compile(minimize(range(compose(fa(FromSt),fa(Tr)))),ToSt),
...
augment_reachable(Line,[[Line,ReachAtLine]|ReachedL],NewSt,[[Line,ReachAtLine2]|ReachedL]) :-
  fsa_regex_compile(minimize(union(fa(ReachAtLine),fa(NewSt))),ReachAtLine2),
  !.
...
septr_do_one_run_ph5(N,FromSt,ToLine,ToSt,ToAbSt,Tr,ITr,TrL,ReachedL,[ToErr,ToBackErr],Result,Value) :-
  %% write('... phase 5'),nl,
  write('Computing pre...'),nl,
  fsa_regex_compile(minimize(range(compose(fa(ToErr),fa(ITr)))),BackErr),
...
septr_one_run(IniLine,IniSt,TrL,ReachedL,Result2,Value2) :-
  ...
  ( (Result1=good, !,Result2=Result1,Value2=Value1);
    ...
    (Value1=[FromErr,BackErr],
     ...
     ( (empty(IniBadSt), !, Result=new,Value=[Init,BackErr]); %% A big or small
       %% (empty(IniBadSt), !, Result=new,Value=[Init,FromErr]); %% new pred (^^^ too)
       (Result=bad,Value=IniBadSt) ) ) ).
...
septr_do_asmc(IniLine,IniSt,Empty,TrL) :-
  ...
  ( (Result=good, !, write('----- Property holds!'),nl,
    ...
    (Result=bad, !,fsa_write_file('error_from',Value),write('----- Property broken!'),nl);
    (Value=[CritConcState,NewPred],add_new_pred(CritConcState,NewPred),septr_do_asmc(IniLine,IniSt,Empty,TrL)) ),
  !.
...
aut_loop_det_n_stp_via_dt(0,St1,_,StViaL1,TrViaL1,TrL,_,_,N,St,NewN1,NewN2,LoopsL) :-
  reverse(TrViaL1,TrViaL2),
  aut_loop_det_snd_n_stp(N,St1,TrL,StViaL1,TrViaL2,TrL,St,St1,TrViaL2,NewN1,NewN2,LoopsL),
  !.



aut_loop_det_n_stp_via_dt(N1,St1,TrToDoL1,StViaL1,TrViaL1,TrL,DtSymL,SepSym,N,St,NewN1,NewN3,LoopsL3) :-
  N1>0,
  TrToDoL1=[trans(St1,A,St2)|TrToDoL2],
  not(member(St2,StViaL1)),
  fsa_preds:conjunction(A,in(DtSymL),_),
  N2 is N1-1,
  aut_loop_det_n_stp_via_sep(N2,St2,TrL,[St2|StViaL1],[A|TrViaL1],TrL,DtSymL,SepSym,N,St,NewN1,NewN2,LoopsL1),
  aut_loop_det_n_stp_via_dt(N1,St1,TrToDoL2,StViaL1,TrViaL1,TrL,DtSymL,SepSym,N,St,NewN2,NewN3,LoopsL2),
  append(LoopsL1,LoopsL2,LoopsL3),
  !.
