...
self_loop_acclr(A,B1,B2) :-
  ...
  retract(bt(_)),
  retract(loopon(SL)),
  tr_to_loop(SL,SL,Bt1L,['_old',/]),
  %% write('Acceleration is adding the following loops: '),write(['_old',/]),nl,
  append(Bt1L,['_old',/],Bt2L),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Abstraction by finite-length traces as well as languages
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
successor_via_ntimes_a_letter(0,Q,_,_,Visited,Visited,Q) :- !.

successor_via_ntimes_a_letter(N,Q1,A,TrL,Visited,AllVisited,Q3) :-
  successor_via_letter(Q1,A,TrL,Q2),
  not(member(Q2,Visited)),
  NN is N-1,
  successor_via_ntimes_a_letter(NN,Q2,A,TrL,[Q2|Visited],AllVisited,Q3).
...
successor_via_ntimes_a_word(0,Q,_,_,Visited,Visited,Q) :- !.

successor_via_ntimes_a_word(N,Q1,AL,TrL,Visited,AllVisited,Q3) :-
  successor_via_word(Q1,AL,TrL,Q2),
  not(member(Q2,Visited)),
  NN is N-1,
  successor_via_ntimes_a_word(NN,Q2,AL,TrL,[Q2|Visited],AllVisited,Q3).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The main loop of accelerating via unwound self-loops
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

do_accel_unwound_self_loops(N,N,_,_,TrL,TrL) :- !.

do_accel_unwound_self_loops(M,N,UnwoundTimes,LoopLetter,TrL,[trans(X,LoopLetter,X)|NewTrL]) :-
  successor_via_ntimes_a_letter(UnwoundTimes,M,LoopLetter,TrL,[M],Visited,Y),
  ( (UnwoundTimes=1, !, X=M);
     %% (successor_via_letter(M,LoopLetter,TrL,X)) ),
     (Visited=[Y,X|_]) ),
     %% (X=Y) ),
  write('!!! Adding a self-loop at:'),write(X),write(' !!!'),nl,
  MM is M+1,
  do_accel_unwound_self_loops(MM,N,UnwoundTimes,LoopLetter,TrL,NewTrL),
  !.

do_accel_unwound_self_loops(M,N,UnwoundTimes,LoopLetter,TrL,NewTrL) :-
  MM is M+1,
  do_accel_unwound_self_loops(MM,N,UnwoundTimes,LoopLetter,TrL,NewTrL).
...
accel_unwound_self_loops(LoopLetter,States,AccelStates) :-
  ...
  do_accel_unwound_self_loops(0,StN,UnwoundTimes,LoopLetter,TrL,NewTrL),
...
do_accel_unwound_loops(StN,StN,_,_,_,AugmStN,TrL,AugmStN,[]) :- !.

do_accel_unwound_loops(N,StN,UnwoundTimes,['_old',/],LoopLen,AugmStN,TrL,NewStN,NewTrL) :-
  successor_via_ntimes_a_word(UnwoundTimes,N,['_old',/],TrL,[N],Visited,Y),
  ( (UnwoundTimes=1, !, X=N);
     %% successor_via_word(N,['_old',/],TrL,X) ),
     (Visited=[Y,X|_]) ),
     %% (X=Y) ),
  write('!!! Adding a loop at:'),write(X),write(' !!!'),nl,
  add_accel_unwound_loop(AugmStN,X,['_old',/],NewTrL1),
  NewAugmStN is AugmStN + LoopLen - 1,
  NN is N+1,
  do_accel_unwound_loops(NN,StN,UnwoundTimes,['_old',/],LoopLen,NewAugmStN,TrL,NewStN,NewTrL2),
  append(NewTrL1,NewTrL2,NewTrL),
  !.

do_accel_unwound_loops(N,StN,UnwoundTimes,['_old',/],LoopLen,AugmStN,TrL,NewStN,NewTrL) :-
  NN is N+1,
  do_accel_unwound_loops(NN,StN,UnwoundTimes,['_old',/],LoopLen,AugmStN,TrL,NewStN,NewTrL).

%-------------------------------------------------------------------------------

accel_unwound_loops(['_old',/],States,AccelStates) :-
  States=fa(PredModule,StN,IniL,FinL,TrL,[]),
  length(['_old',/],LoopLen),
  unwound_times(UnwoundTimes),
  write('Accelerating via '),write(['_old',/]),write(' loops unwound '),
    write(UnwoundTimes),write(' times...'),nl,
  do_accel_unwound_loops(0,StN,UnwoundTimes,['_old',/],LoopLen,StN,TrL,NewStN,NewTrL),
  append(TrL,NewTrL,AllTrL),
  fsa_regex_compile(minimize(cleanup(fa(fa(PredModule,NewStN,IniL,FinL,AllTrL,[])))),AccelStates).
...
do_accel_several_unwound_loops([['_old',/]|LoopsRestL],States,AccelStates) :-
  States=fa(PredModule,StN,IniL,FinL,TrL,[]),
  length(['_old',/],LoopLen),
  unwound_times(UnwoundTimes),
  write('Accelerating via '),write(['_old',/]),write(' loops unwound '),
  ...
  do_accel_unwound_loops(0,StN,UnwoundTimes,['_old',/],LoopLen,StN,TrL,NewStN,NewTrL),
  append(TrL,NewTrL,AllTrL),
  do_accel_several_unwound_loops(LoopsRestL,fa(PredModule,NewStN,IniL,FinL,AllTrL,[]),AccelStates).
...
:- sort([!,#,'_old','_bgn','_fst','_snd','_end','_ins',l,x,y,z,'_f','_t',n,e,s,su,u,/,'|',l1,l2,l3,l4,l5,l6,l7,l8,le1,le2,m_f,m_t],S),assertz(sigma(S)).
%%%%%%%%%%%%%%%%%%%%%%%%
...
abstract(State,AbsState) :- accel_unwound_loops(['_old',/],State,AbsState).

%%% A version for debugging:

%% abstract(State,AbsState) :-
%% accel_unwound_loops(['_old',/],State,AbsState),
