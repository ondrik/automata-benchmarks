...
abstr_fw(Conc,Abstr) :-
  ...
  Conc=fa(ConcSym,ConcQN,ConcIniL,ConcFinL,ConcTrL,_),
    ...
    ConcIniL=[ConcIni],
    ini_partition_mod_n(18,ConcIni,ConcQN,ConcTrL,IniClsL),
    ini_fw_classes_mod_n(IniClsL),
...
fl_comp_partition(N,QN,FL,TrL,PartL) :-
  sigma(SigL),
    ...
    ini_partition_mod_n(18,0,QN,TrL,IniClsL1),
    split_ini_part_mod_n_fin_nonfin(IniClsL1,FL,_,IniClsL),

  fl_comp_classes(0,N,QN,FL,SigL,TrL,IniClsL,ClL),
...
do_comp_ini_partition_mod_n(Q1,N,ToDoL,DoneL,[trans(Q1,_,Q2)|TrL],NewToDoL,NewDoneL) :-
  ( (member([Q2,_],DoneL), !,
     do_comp_ini_partition_mod_n(Q1,N,ToDoL,DoneL,TrL,NewToDoL,NewDoneL));
    (
     ...
     do_comp_ini_partition_mod_n(Q1,N,[[Q2,N]|ToDoL],[[Q2,Cl]|DoneL],TrL,NewToDoL,NewDoneL)) ),
  !.

do_comp_ini_partition_mod_n(Q,N,ToDoL,DoneL,[_|TrL],NewToDoL,NewDoneL) :-
  do_comp_ini_partition_mod_n(Q,N,ToDoL,DoneL,TrL,NewToDoL,NewDoneL),
  !.

do_comp_ini_partition_mod_n(_,_,ToDoL,DoneL,[],ToDoL,DoneL).

%------------------------------------------------------------------------------

comp_ini_partition_mod_n(Mod,[[Q,N]|ToDoL],DoneL,TrL,NewDoneL2) :-
  NN is (N+1) mod Mod,
  do_comp_ini_partition_mod_n(Q,NN,ToDoL,DoneL,TrL,NewToDoL1,NewDoneL1),
  comp_ini_partition_mod_n(Mod,NewToDoL1,NewDoneL1,TrL,NewDoneL2),
  !.

comp_ini_partition_mod_n(_,[],DoneL,_,DoneL).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

do_collect_ini_partition_mod_n(N,[[Q,N]|ReachL],UnusedL,[Q|CollectedL]) :-
  do_collect_ini_partition_mod_n(N,ReachL,UnusedL,CollectedL),
  !.

do_collect_ini_partition_mod_n(N1,[[Q,N2]|ReachL],[[Q,N2]|UnusedL],CollectedL) :-
  do_collect_ini_partition_mod_n(N1,ReachL,UnusedL,CollectedL),
  !.

do_collect_ini_partition_mod_n(_,[],[],[]).

%------------------------------------------------------------------------------

collect_ini_partition_mod_n(N,Mod,ReachL,ResultClL) :-
   N<Mod,
   !,
   do_collect_ini_partition_mod_n(N,ReachL,UnusedL,CollectedL),
   NN is N+1,
   collect_ini_partition_mod_n(NN,Mod,UnusedL,IniClsL),
   ( (CollectedL=[], !, ResultClL=IniClsL);
     ResultClL=[[N,CollectedL]|IniClsL] ),
   !.

collect_ini_partition_mod_n(Mod,Mod,[],[]).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

ini_partition_mod_n(Mod,Q0,QN,TrL,IniClsL) :-
  comp_ini_partition_mod_n(Mod,[[Q0,0]],[[Q0,0]],TrL,ReachL),



    collect_ini_partition_mod_n(0,QN,ReachL,IniClsL),
...
do_split_ini_part_mod_n_fin_nonfin([Q|QL],FinL,[Q|QFinL],QNonFinL) :-
  member(Q,FinL),
  !,
  do_split_ini_part_mod_n_fin_nonfin(QL,FinL,QFinL,QNonFinL),
  !.

do_split_ini_part_mod_n_fin_nonfin([Q|QL],FinL,QFinL,[Q|QNonFinL]) :-
  do_split_ini_part_mod_n_fin_nonfin(QL,FinL,QFinL,QNonFinL),
  !.

do_split_ini_part_mod_n_fin_nonfin([],_,[],[]).

%------------------------------------------------------------------------------

split_ini_part_mod_n_fin_nonfin([[_,QL]|ClsL],FinL,N,NewClsL) :-
  do_split_ini_part_mod_n_fin_nonfin(QL,FinL,QFinL,QNonFinL),
  split_ini_part_mod_n_fin_nonfin(ClsL,FinL,N1,NewClsL1),
  ( (QFinL=[], !, N is N1+1, NewClsL=[[N1,QNonFinL]|NewClsL1]);
    ( (QNonFinL=[], !, N is N1+1, NewClsL=[[N1,QFinL]|NewClsL1]);
      (N2 is N1+1, N is N2+1, NewClsL=[[N1,QFinL],[N2,QNonFinL]|NewClsL1]) ) ),
  !.

split_ini_part_mod_n_fin_nonfin([],_,0,[]).
...
fb_comp_partition(N,QN,IL,TrL,PartL) :-
  sigma(SigL),
    ...
    ini_partition_mod_n(18,0,QN,TrL,IniClsL1),
    split_ini_part_mod_n_fin_nonfin(IniClsL1,IL,_,IniClsL),

  fb_comp_classes(0,N,QN,IL,SigL,TrL,IniClsL,ClL),
  maplist(fl_take_snd,ClL,PartL),
  !.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ft_comp_partition(N,QN,TrL,PartL) :-
  sigma(SigL),
    ...
    ini_partition_mod_n(18,0,QN,TrL,IniClsL),

  fl_comp_classes(0,N,QN,QL,SigL,TrL,IniClsL,ClL),
  maplist(fl_take_snd,ClL,PartL),
  !.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
abstr_fl_refby_ft(Conc,Abstr) :-
  ...
  sigma(SigL),
    ...
    ini_partition_mod_n(18,0,ConcQN,ConcTrL,IniClsL1),
    split_ini_part_mod_n_fin_nonfin(IniClsL1,FL,_,IniClsL),

  fl_comp_classes(0,Lim,ConcQN,ConcFinL,SigL,ConcTrL,IniClsL,ClL1),
  N=1,
...
ftb_comp_partition(N,QN,TrL,PartL) :-
  ...
  fb_comp_classes(0,N,QN,QL,SigL,TrL,IniClsL,ClL),
  maplist(fl_take_snd,ClL,PartL),
  !.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
refine_abstraction(CritConcState,NewPred) :-
  ...
  retract(fin_lang_up_to(OldLim)), %% !!! Switch off retract(pred_lang(PredLangL)) %% Choose this or ...
    NewLim is OldLim+18, %% Choose this or ...
