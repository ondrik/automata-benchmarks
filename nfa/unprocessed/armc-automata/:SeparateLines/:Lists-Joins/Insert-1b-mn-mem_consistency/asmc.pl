...
is_included(A,B) :-
  empty(E),
  fsa_regex_compile(min_mh_mb(intersect(fa(A),complement(fa(B)))),E),
  !.
...
do_ini_fw_classes_mod_n(N,[Q|QL]) :-
  assertz(label(Q,[N])),
  do_ini_fw_classes_mod_n(N,QL),
  !.

do_ini_fw_classes_mod_n(_,[]).

%...............................................................................

ini_fw_classes_mod_n([[N,QL]|IniClsL]) :-
  do_ini_fw_classes_mod_n(N,QL),
  ini_fw_classes_mod_n(IniClsL),
  !.

ini_fw_classes_mod_n([]).
...
abstr_fw(Conc,Abstr) :-
  ...
  #ifdef RefPartMod
    ConcIniL=[ConcIni],
    ini_partition_mod_n(RefPartMod,ConcIni,ConcQN,ConcTrL,IniClsL),
    ini_fw_classes_mod_n(IniClsL),
  #endif
  do_abstr_fw(Conc,PredL),
  ...
  fsa_regex_compile(min_mh_mb(fa(Abstr1)),Abstr).
...
abstr_fw(Conc,Abstr) :-
  ...
  %% write('Computing the partition...'),nl,
  collect_partition(PartitionL),
  split_class_list_wrt_rep_sym_unnum(PartitionL,ConcTrL,RefndPartL),
  %% write('Partition: '),write(RefndPartL),nl,
  adjust_trans(RefndPartL,ConcTrL,NewConcTrL),
  adjust_st(RefndPartL,ConcFinL,NewConcFinL),
  adjust_st(RefndPartL,ConcIniL,NewConcIniL),
  fsa_construct_rename_states(ConcSym,NewConcIniL,NewConcFinL,NewConcTrL,[],Abstr1),
  fsa_regex_compile(min_mh_mb(fa(Abstr1)),Abstr).
...
fl_comp_partition(N,QN,FL,TrL,PartL) :-
  sigma(SigL),
  fl_comp_nonf(0,QN,FL,NonFL),
  split_class_list_wrt_rep_sym([[0,FL],[1,NonFL]],TrL,RefndIniClL),
  fl_comp_classes(0,N,QN,FL,SigL,TrL,RefndIniClL,ClL),
  maplist(fl_take_snd,ClL,PartL),
  !.
...
abstr_fl(Conc,Abstr) :-
  ...
  fl_comp_partition(Lim,ConcQN,ConcFinL,ConcTrL,PartitionL1),
  #ifndef RefPartMod
    PartitionL=PartitionL1,
  #else
    ConcIniL=[ConcIni],
    partition_mod_n(RefPartMod,ConcIni,ConcQN,ConcTrL,ModClsL),
    refine_part_list(PartitionL1,ModClsL,PartitionL),
  #endif
  %% write('Partition: '),write(PartitionL),nl,
  adjust_trans(PartitionL,ConcTrL,NewConcTrL),
  adjust_st(PartitionL,ConcFinL,NewConcFinL),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Refining the initial partition not to collapse not corresponding elements of NDDs, ...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

do_comp_partition_mod_n(Q1,N,ToDoL,DoneL,[trans(Q1,_,Q2)|TrL],NewToDoL,NewDoneL) :-
  ( (member([Q2,_],DoneL), !,
     do_comp_partition_mod_n(Q1,N,ToDoL,DoneL,TrL,NewToDoL,NewDoneL));
    (
     #ifndef JustExactMod
       Cl=N,
     #else
       ( (N=0, !, Cl=0); Cl=Q2),
     #endif
     do_comp_partition_mod_n(Q1,N,[[Q2,N]|ToDoL],[[Q2,Cl]|DoneL],TrL,NewToDoL,NewDoneL)) ),
  !.

do_comp_partition_mod_n(Q,N,ToDoL,DoneL,[_|TrL],NewToDoL,NewDoneL) :- 
  do_comp_partition_mod_n(Q,N,ToDoL,DoneL,TrL,NewToDoL,NewDoneL),
  !.

do_comp_partition_mod_n(_,_,ToDoL,DoneL,[],ToDoL,DoneL).

%------------------------------------------------------------------------------

comp_partition_mod_n(Mod,[[Q,N]|ToDoL],DoneL,TrL,NewDoneL2) :-
  NN is (N+1) mod Mod,
  do_comp_partition_mod_n(Q,NN,ToDoL,DoneL,TrL,NewToDoL1,NewDoneL1),
  comp_partition_mod_n(Mod,NewToDoL1,NewDoneL1,TrL,NewDoneL2),
  !.

comp_partition_mod_n(_,[],DoneL,_,DoneL).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

do_collect_partition_mod_n(N,[[Q,N]|ReachL],UnusedL,[Q|CollectedL]) :-
  do_collect_partition_mod_n(N,ReachL,UnusedL,CollectedL),
  !.

do_collect_partition_mod_n(N1,[[Q,N2]|ReachL],[[Q,N2]|UnusedL],CollectedL) :-
  do_collect_partition_mod_n(N1,ReachL,UnusedL,CollectedL),
  !.

do_collect_partition_mod_n(_,[],[],[]).

%------------------------------------------------------------------------------

collect_partition_mod_n(N,Mod,ReachL,ResultClL) :-
   N<Mod,
   !,
   do_collect_partition_mod_n(N,ReachL,UnusedL,CollectedL),
   NN is N+1,
   collect_partition_mod_n(NN,Mod,UnusedL,IniClsL),
   ( (CollectedL=[], !, ResultClL=IniClsL);
     ResultClL=[[N,CollectedL]|IniClsL] ),
   !.

collect_partition_mod_n(Mod,Mod,[],[]).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

partition_mod_n(Mod,Q0,QN,TrL,IniClsL) :- 
  comp_partition_mod_n(Mod,[[Q0,0]],[[Q0,0]],TrL,ReachL),
  #ifndef JustExactMod
    collect_partition_mod_n(0,Mod,ReachL,IniClsL),
  #else
    collect_partition_mod_n(0,QN,ReachL,IniClsL),
  #endif  
  !.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

do_refine_part(N,[Q|QL],RefWrtL,[Q|InNL],RestL) :-
  in_set_of(Q,N,RefWrtL),
  !,
  do_refine_part(N,QL,RefWrtL,InNL,RestL),
  !.
  
do_refine_part(N,[Q|QL],RefWrtL,InNL,[Q|RestL]) :-
  do_refine_part(N,QL,RefWrtL,InNL,RestL),
  !.
...
refine_part([Q|QL],RefWrtL,[[Q|WithQL]|NewClsL]) :-
  in_set_of(Q,N,RefWrtL),
  do_refine_part(N,QL,RefWrtL,WithQL,RestL),
  refine_part(RestL,RefWrt,NewClsL),
  !.
...
refine_part_list([Cls|ClsL],RefWrtL,NewClsL) :-
  refine_part(Cls,RefWrtL,NewClsL1),
  refine_part_list(ClsL,RefWrtL,NewClsL2),
  append(NewClsL1,NewClsL2,NewClsL),
  !.
...
fb_comp_partition(N,QN,IL,TrL,PartL) :-
  sigma(SigL),
  fl_comp_nonf(0,QN,IL,NonIL),                                 %% NonIL for fb...
  split_class_list_wrt_rep_sym([[0,IL],[1,NonIL]],TrL,RefndIniClL),
  fb_comp_classes(0,N,QN,IL,SigL,TrL,RefndIniClL,ClL),
  maplist(fl_take_snd,ClL,PartL),
  !.
...
abstr_fb(Conc,Abstr) :-
  ...
  fb_comp_partition(Lim,ConcQN,ConcIniL,ConcTrL,PartitionL1),
  #ifndef RefPartMod
    PartitionL=PartitionL1,
  #else
    ConcIniL=[ConcIni],
...
ft_comp_partition(N,QN,TrL,PartL) :-
  sigma(SigL),
  ft_gen_st_list(QN,QL),
  split_class_list_wrt_rep_sym([[0,QL]],TrL,RefndIniClL),
  fl_comp_classes(0,N,QN,QL,SigL,TrL,RefndIniClL,ClL),
  maplist(fl_take_snd,ClL,PartL),
  !.
...
abstr_ft(Conc,Abstr) :-
  ...
  ft_comp_partition(Lim,ConcQN,ConcTrL,PartitionL1),
  #ifndef RefPartMod
    PartitionL=PartitionL1,
  #else
    ConcIniL=[ConcIni],
...
abstr_fl_refby_ft(Conc,Abstr) :-
  ...
  maplist(fl_take_snd,ClL2,PartitionL1),
  #ifndef RefPartMod
    PartitionL=PartitionL1,
  #else
    ConcIniL=[ConcIni],
...
abstr_fl_refby_ft(Conc,Abstr) :-
  ...
  sigma(SigL),
  fl_comp_nonf(0,ConcQN,ConcFinL,NonFL),
  split_class_list_wrt_rep_sym([[0,ConcFinL],[1,NonFL]],ConcTrL,RefndIniClL),
  write('Computing the FL partition wrt. languages up to '),write(Lim),write(' letters...'),nl,
  fl_comp_classes(0,Lim,ConcQN,ConcFinL,SigL,ConcTrL,RefndIniClL,ClL1),  
  N=1,
  write('Computing the FT partition wrt. languages up to '),write(N),write(' letters...'),nl,
  ft_gen_st_list(ConcQN,QL),
  fl_comp_classes(0,N,ConcQN,QL,SigL,ConcTrL,ClL1,ClL2),
...
ftb_comp_partition(N,QN,TrL,PartL) :-
  ...
  ft_gen_st_list(QN,QL),
  split_class_list_wrt_rep_sym([[0,QL]],TrL,RefndIniClL),
  fb_comp_classes(0,N,QN,QL,SigL,TrL,RefndIniClL,ClL),
  maplist(fl_take_snd,ClL,PartL),
  !.
...
abstr_ftb(Conc,Abstr) :-
  ...
  ftb_comp_partition(Lim,ConcQN,ConcTrL,PartitionL1),
  #ifndef RefPartMod
    PartitionL=PartitionL1,
  #else
    ConcIniL=[ConcIni],
...
fst_to_minfsa(fa(t(fsa_frozen,fsa_frozen),QN,IL,FL,TrL,JL),FA2) :-
  ...
  fsa_regex_compile(min_mh_mb(fa(FA1)),FA2).
...
refine_abstraction(CritConcState,NewPred) :-
...
#if defined (Intrs) || defined (IntrsTail) || defined (IntrsTailKey) || defined (IntrsHead) || defined (IntrsHeadKey)
  retract(pred_lang(PredLangL)),  %% Must be switched off for abstr_fl, but on otherwise...
#endif
  %-------------
#ifdef Intrs
 #ifdef AddFlColl
  abstr_fl(NewPred,NewPredToStore),
  #ifndef AddBoth
    assertz(pred_lang([NewPredToStore|PredLangL])),
  #else
    assertz(pred_lang([NewPred,NewPredToStore|PredLangL])),
  #endif
 #elif AddFtColl
  abstr_ft(NewPred,NewPredToStore),
  #ifndef AddBoth
    assertz(pred_lang([NewPredToStore|PredLangL])),
  #else
    assertz(pred_lang([NewPred,NewPredToStore|PredLangL])),
  #endif
 #elif AddFbColl
  abstr_fb(NewPred,NewPredToStore),
  #ifndef AddBoth
    assertz(pred_lang([NewPredToStore|PredLangL])),
  #else
    assertz(pred_lang([NewPred,NewPredToStore|PredLangL])),
  #endif  
 #elif AddFbtColl
  abstr_ftb(NewPred,NewPredToStore),
  #ifndef AddBoth
    assertz(pred_lang([NewPredToStore|PredLangL])),
  #else
    assertz(pred_lang([NewPred,NewPredToStore|PredLangL])),
  ...
  abstr_ftb(NewPred,NewPredToStore),
 #else
  NewPredToStore=NewPred,
 #endif
  common_prefix(CritConcState,NewPredToStore,ImpStL),                                                %% Choose this or ...
    write('These are the important states of the new predicate: '),write(ImpStL),nl,
    assertz(pred_lang([[NewPredToStore,ImpStL]|PredLangL])),
    augment_summary_bad(NewPred), %% May be switched off.
#endif
  %-------------
  ...
  NewPredToStore=NewPred,
 #endif
  common_prefix(CritConcState,NewPredToStore,ImpStL),  				      
    write('These are the important states of the new predicate: '),write(ImpStL),nl,                 %% Choose this or ...
    key_states(ImpStL,CritConcState,NewPredToStore,PredLangL,QKeyL),
    NewRedPred=NewPredToStore,NewQKeyL=QKeyL,  			                   %% Choose this or vvv
    %% after a modification reduce_pred(NewPredToStore,QKeyL,NewRedPred,NewQKeyL),    %% Choose this or ^^^
  ...
  NewPredToStore=NewPred,
 #endif
  common_suffix(CritConcState,NewPredToStore,ImpStL),                                                %% Choose this or ...
    write('These are the important states of the new predicate: '),write(ImpStL),nl,
    assertz(pred_lang([[NewPredToStore,ImpStL]|PredLangL])),
    augment_summary_bad(NewPred), %% May be switched off.
#endif
  ...
  common_suffix(CritConcState,NewPredToStore,ImpStL),  					             %% Choose this or ...
    ...
    key_states(ImpStL,CritConcState,NewPredToStore,PredLangL,QKeyL),
    NewRedPred=NewPredToStore,NewQKeyL=QKeyL,  		     %% Choose this or vvv
    %% reduce_pred(NewPredToStore,QKeyL,NewRedPred,NewQKeyL),  	%% Choose this or ^^^
    assertz(pred_lang([[NewRedPred,NewQKeyL]|PredLangL])),
    augment_summary_bad(NewPred), %% May be switched off.
#endif
  %-------------
...
#ifdef IncOne
  retract(fin_lang_up_to(OldLim)), %% !!! Switch off retract(pred_lang(PredLangL))		     %% Choose this or ...
    NewLim is OldLim+1,     %% Choose this or ...
    write('Increasing the limit on the lenghts of the words considered: '),write(OldLim),write(' -> '),write(NewLim),nl,
    assertz(fin_lang_up_to(NewLim)),
    augment_summary_bad(NewPred), %% May be switched off.
#endif
...
#ifdef IncSp
  retract(fin_lang_up_to(OldLim)), %% !!! Switch off retract(pred_lang(PredLangL))		     %% Choose this or ...
    NewLim is OldLim+IncSp,     %% Choose this or ...
    write('Increasing the limit on the lenghts of the words considered: '),write(OldLim),write(' -> '),write(NewLim),nl,
    assertz(fin_lang_up_to(NewLim)),
    augment_summary_bad(NewPred), %% May be switched off.
#endif
  %-------------
#ifdef IncConc
  CritConcState=fa(_,ConcSize,_,_,_,_),
  retract(fin_lang_up_to(OldLim)), %% !!! Switch off retract(pred_lang(PredLangL))		     %% Choose this or ...
    NewLim is OldLim+ConcSize,
    write('Increasing the limit on the lenghts of the words considered: '),write(OldLim),write(' -> '),write(NewLim),nl,
    assertz(fin_lang_up_to(NewLim)),
    augment_summary_bad(NewPred), %% May be switched off.
#endif
  ...
  CritConcState=fa(_,ConcSize,_,_,_,_),
  retract(fin_lang_up_to(OldLim)), %% !!! Switch off retract(pred_lang(PredLangL))		     %% Choose this or ...
    NewLim is OldLim+(ConcSize//2),  %% Choose this or ...
    write('Increasing the limit on the lenghts of the words considered: '),write(OldLim),write(' -> '),write(NewLim),nl,
    assertz(fin_lang_up_to(NewLim)),
    augment_summary_bad(NewPred), %% May be switched off.
#endif
  ...
  %%   augment_summary_bad(NewPred), %% May be switched off.
  %-------------
#if defined(AccelViaUnwSelfLoops) || defined(AccelViaUnwSelfLoopsInv) || defined(InvAccelViaUnwSelfLoopsInv)
  retract(unwound_times(UnwoundTimes)),
  NewUnwoundTimes is UnwoundTimes+1,
  assertz(unwound_times(NewUnwoundTimes)),
#endif
  %-------------
#if defined(AccelViaUnwLoops) || defined(AccelViaUnwLoopsInv) || defined(InvAccelViaUnwLoopsInv) || defined(AccelViaSeveralUnwLoops)
  retract(unwound_times(UnwoundTimes)),
  NewUnwoundTimes is UnwoundTimes+1,
  assertz(unwound_times(NewUnwoundTimes)),
#endif
  %-------------
#if defined(AutoDetLoopsFrom) || defined(AutoDetDtLoopsFrom)
  retract(unwound_times(UnwoundTimes)),
  NewUnwoundTimes is UnwoundTimes+1,
  assertz(unwound_times(NewUnwoundTimes)),
#endif
  ...
  retract(all_pos_loops_up_to(AllLoopsUpTo1)),
  AllLoopsUpTo2 is AllLoopsUpTo1 + 1,
  assertz(all_pos_loops_up_to(AllLoopsUpTo2)),
  retract(all_pos_loops(_)),
  all_pos_loops_gen_loop(AllLoopsUpTo2,DtSymL,DtSymL,SepSym,[],AllLoopsL),  
  retract(all_pos_loops(AllLoopsL)),
#endif
  %-------------
...
fire_n_tr([Tr|TrL],From,UnionTo) :-
  fire_n_tr(TrL,From,AuxUnionTo),
  fsa_regex_compile(min_mh_mb(range(compose(fa(From),fa(Tr)))),To),
  fsa_regex_compile(min_mh_mb(union(fa(To),fa(AuxUnionTo))),UnionTo),
  !.
...
do_one_run(N,AbsReach,Result,Value) :-
 ...
 #ifndef GoLenPr
  #ifdef NTr
    fire_n_tr(TR,AbsReach,NewReach),   %% Taking a union of N sub-transducers
  #elif TwoTr 
    tr2(TR2),                          %% Two phases fired in a series
    fsa_regex_compile(min_mh_mb(range(compose(fa(AbsReach),fa(TR)))),NewReach1),
    fsa_regex_compile(min_mh_mb(range(compose(fa(NewReach1),fa(TR2)))),NewReach),
  #else
    fsa_regex_compile(min_mh_mb(range(compose(fa(AbsReach),fa(TR)))),NewReach),
      %% mmdff(AbsReach,MmdffOld),write('MmdffOld: '),write(MmdffOld),write(', '),
      %% mmdff(NewReach,MmdffNew),write('MmdffNew: '),write(MmdffNew),nl,
  #endif
 #else
  fsa_regex_compile(min_mh_mb(range(compose(fa(AbsReach),fa(TR)))),NewReachAux),
  fsa_regex_atom_compile('{?,[]: \'_b\', \'_b\':[]}*',TrRePad),
  fsa_regex_compile(min_mh_mb(range(compose(fa(NewReachAux),fa(TrRePad)))),NewReach),
 #endif
#else
  ...
  #ifdef GenAbsReached
    fsa_write_file('conc_reached',NewReach),
  #endif 
  ( (empty(ImmReachBad), !,do_one_run_ph2(NN,AbsReach,NewReach,Result,Value));
    (write('XXXXXXXXX'),nl,
     #ifdef GenErrReached
      fsa_write_file('error_reached',ImmReachBad), %% trace,
     #endif
     Result=bad,Value=[ImmReachBad,ImmReachBad,NewReach]) ).
...
%%   AbsReach=fa(_,NS1,_,_,T1,_),length(T1,NT1),
%%   write('Abstract source automaton: '),write(NS1),write(','),write(NT1),nl,
%%   fsa_regex_compile(min_mh_mb(range(compose(fa(AbsReach),fa(TR)))),OneStepReach),
...
do_one_run_ph2(N,AbsReach,NewReach,Result,Value) :-
  ...
  #ifdef GenAbsReached
    fsa_write_file('abs_reached',NewAbsReach),
  #endif  
  ( (empty(AbsImmBad), !,
    ...
    (write('YYYYYYYYY'),nl,
     #ifdef GenErrReached
       fsa_write_file('error_reached',AbsImmBad), %% trace,
     #endif
     %% fsa_write_file(aaa,NewAbsReach), fsa_write_file(ccc,NewReach), trace,
...
#ifndef TrClosure
     #ifdef NTr
       fire_n_tr(ITR,NextReachBad1,SrcBad),
     #elif TwoTr
       itr2(ITR2),
       fsa_regex_compile(min_mh_mb(range(compose(fa(NextReachBad1),fa(ITR)))),SrcBad1),
       fsa_regex_compile(min_mh_mb(range(compose(fa(SrcBad1),fa(ITR2)))),SrcBad),
     #else
       fsa_regex_compile(min_mh_mb(range(compose(fa(NextReachBad1),fa(ITR)))),SrcBad),
     #endif
#else
...
do_one_run_ph4(N,NewReach,NewAbsReach,NewPred,NextReach,Result,Value) :-
  refine_abstraction(NextReach,NewPred),
  retract(run(Run)),
  NewRun is Run+1,
  assertz(run(NewRun)),
...
inverse_transitions([Tr|TrL],[ITr|ITrL]) :-
  fsa_regex_compile(inverse(fa(Tr)),ITr),
  inverse_transitions(TrL,ITrL),
  !.
...
#ifndef TrClosure
  #ifdef NTr
    inverse_transitions(TR,ITR),
    assertz(itr(ITR)),
  #elif TwoTr
    tr2(TR2),
    fsa_regex_compile(inverse(fa(TR)),ITR),
    fsa_regex_compile(inverse(fa(TR2)),ITR2),
    assertz(itr(ITR2)),                        %%% Note: the roles of tr1 x tr2 change when going backwards!!!
    assertz(itr2(ITR)),    
  #else
    fsa_regex_compile(inverse(fa(TR)),ITR),
    assertz(itr(ITR)),    
...
#ifndef TrClosure
  #ifdef NTr
    inverse_transitions(ITR,TR),
    assertz(tr(TR)),
  #elif TwoTr
    itr2(ITR2),
    fsa_regex_compile(inverse(fa(ITR)),TR),
    fsa_regex_compile(inverse(fa(ITR2)),TR2),
    assertz(tr(TR2)),                        %%% Note: the roles of tr1 x tr2 change when going backwards!!!
    assertz(tr2(TR)),    
  #else
    fsa_regex_compile(inverse(fa(ITR)),TR),
    assertz(tr(TR)),   
...
%%  fsa_regex_compile(min_mh_mb(fa(fa(X,S,I,F,T2,Y))),A).
...
#else
lrn_qtrans_upto(M,N,TL) :-
  MM is M+1,
  sigma(SL),
  lrn_gen_trans(SL,M,MM,TL1),
  lrn_qtrans_upto(MM,N,TL2),
  append(TL1,TL2,TL).
#endif
...
lrn_one_step_on(From,TR,To) :-
    ...
    fsa_regex_compile(min_mh_mb(range(compose(fa(From),fa(TR)))),To),
  #else
    fsa_to_fst(From,FromTrsd),
    %%% write('>> Composing transducers... '), nl,
    fsa_regex_compile(compose(fa(FromTrsd),fa(TR)),ToTrsd),
    fst_to_minfsa(ToTrsd,To),
  #endif
  true.
...
lrn_do_nlenpr_reachable_upto_n(From,AllUpToN,TR,Reachable) :-
  %%% write('>> One step on... '), nl,
  fsa_regex_compile(min_mh_mb(range(compose(fa(From),fa(TR)))),TR_From),
...
lrn_do_collapse(State,AbsState) :-
  ...
  #elif FtColl
    abstr_ft(State,AbsState),
  #elif FbColl  
    abstr_fb(State,AbsState),
  #elif FbtColl
    abstr_ftb(State,AbsState),
  #elif TBColl
    abs_tracht_barzd(State,AbsState),
  #elif FlFtColl
    abstr_fl_ft(State,AbsState),
  #elif FlRefByFtColl
...
lrn_try_collapse(Reach,Init,Bad,TR,AbsReach) :- 
  ...
  %% fsa_regex_compile(min_mh_mb(min_mh_mb(range(compose(fa(AbsReach),fa(TR))))),AbsReach), %% Choose this or ...
  %% fsa_regex_compile(min_mh_mb(range(compose(fa(AbsReach),fa(TR)))),AbsReach),          %% Choose this or ^^^
...
lrn_try_length_N(N,ExitCode,Result) :-
  ...
  #else
    lrn_nlenpr_reachable_upto_n(Init,N,TR,Reach),
    %%%% fsa_write_file('lrn_do_nlenpr_reachable_upto_n',Reach), trace,
  #endif
  write('- Intersecting with bad...'), nl,
...
lrn_try_length_N(N,ExitCode,Result) :-
  ...
  statistics(cputime,[T11,_]),
    ...
    %%%% fsa_write_file('lrn_do_nlenpr_reachable_upto_n',Reach), trace,
  #endif
  statistics(cputime,[T12,_]),
  T1 is (T12-T11)/1000,
  write('... time used in generating: '),write(T1),write(' sec.'),nl,
  %%% write('- Intersecting with bad...'), nl,
  fsa_regex_compile(intersect(fa(Reach),fa(Bad)),ReachBad),
  ( (empty(ReachBad), !,
     statistics(cputime,[T21,_]),
     ( (lrn_try_collapse(Reach,Init,Bad,TR,AbsReach), !);
       (statistics(cputime,[T22,_]),
        T2 is (T22-T21)/1000,
        write('... time used in collapsing: '),write(T2),write(' sec.'),nl,fail) ),
     statistics(cputime,[T22,_]),
     T2 is (T22-T21)/1000,
     write('... time used in collapsing: '),write(T2),write(' sec.'),nl,
     ExitCode=good,
     Result=AbsReach);
    (ExitCode=bad, Result=ReachBad) ),
  !.
...
tb_lang_of_state(N,fa(Pred,NQ,_,FinL,TL,JL),A) :-
  fsa_regex_compile(min_mh_mb(fa(fa(Pred,NQ,[N],FinL,TL,JL))),A).
...
%   fsa_regex_compile(min_mh_mb(fa(fa(Pred,NQ,[0],FinL,NewTrl,JL))),NewAut),
...
abs_tracht_barzd(fa(Pred,NQ,[0],FinL,TrL,JL),NewAut) :-
  ...
  gen_conf_up_to(N), 
  #ifdef TBFlColl
    tb2_fl_comp_classes(NQ,FinL,TrL,ClLL),
  #elif TBFtColl
    tb2_ft_comp_classes(NQ,TrL,ClLL),
  #elif TBFbColl
    tb2_fb_comp_classes(NQ,[0],TrL,ClLL),
  #elif TBFbtColl
    tb2_fbt_comp_classes(NQ,TrL,ClLL),
  #endif
  tb2_explore_automaton([[0,0]],N,ClLL,[0],TrL,NewTrl),
  %% fsa_regex_compile(min_mh_mb(fa(fa(Pred,NQ,[0],FinL,NewTrl,JL))),NewAut),   %% Choose this or vvv
...
do_accel_unwound_self_loops(StN,StN,_,_,TrL,TrL,0) :- !.

do_accel_unwound_self_loops(St,StN,UnwoundTimes,LoopLetter,TrL,[trans(X,LoopLetter,X)|NewTrL],1) :-
  successor_via_ntimes_a_letter(UnwoundTimes,St,LoopLetter,TrL,[St],Visited),
  ( (UnwoundTimes=1, !, X=St);
     %% (successor_via_letter(St,LoopLetter,TrL,X)) ),
     (Visited=[_,X|_]) ),
     %% (Visited=[X|_]) ),
  write('!!! Adding a self-loop at:'),write(X),write(' !!!'),nl,
  NextSt is St+1,
  do_accel_unwound_self_loops(NextSt,StN,UnwoundTimes,LoopLetter,TrL,NewTrL,_),
  !.
  
do_accel_unwound_self_loops(St,StN,UnwoundTimes,LoopLetter,TrL,NewTrL,AccDone) :-
  NextSt is St+1,
  do_accel_unwound_self_loops(NextSt,StN,UnwoundTimes,LoopLetter,TrL,NewTrL,AccDone).

%-------------------------------------------------------------------------------

accel_unwound_self_loops(LoopLetter,States,AccelStates) :-
  States=fa(PredModule,StN,IniL,FinL,TrL,[]),
  unwound_times(UnwoundTimes),
  write('Accelerating via '),write(LoopLetter),write('-self loops unwound '),
    write(UnwoundTimes),write(' times...'),nl,
  do_accel_unwound_self_loops(0,StN,UnwoundTimes,LoopLetter,TrL,NewTrL,AccDone),
  ( (AccDone=0, !, AccelStates=States);
    (fsa_regex_compile(min_mh_mb(cleanup(fa(fa(PredModule,StN,IniL,FinL,NewTrL,[])))),AccelStates)) ).
...
do_accel_unwound_self_loops(St,StN,UnwoundTimes,LoopLetter,TrL,VeryNewStN,VeryNewTrL,1) :-
  successor_via_ntimes_a_letter(UnwoundTimes,St,LoopLetter,TrL,[St],[C,B,A|_]),
  not(member(trans(C,LoopLetter,C),TrL)),
  NextSt is St+1,
  do_accel_unwound_self_loops(NextSt,StN,UnwoundTimes,LoopLetter,TrL,NewStN,NewTrL,_),
  ...
  %%   VeryNewTrL=[trans(A,LoopLetter,NewStN),trans(NewStN,LoopLetter,NewStN)|AddedOutTrL],
  add_out_tr_after_new_self_loop(NewStN,C,NewTrL,AddedOutTrL),                           %% Choose this or ^^^ ... 
    VeryNewTrL=[trans(A,LoopLetter,NewStN),trans(NewStN,LoopLetter,NewStN)|AddedOutTrL],
  !.
  
do_accel_unwound_self_loops(St,StN,UnwoundTimes,LoopLetter,TrL,NewStN,NewTrL,AccDone) :-
  NextSt is St+1,
  do_accel_unwound_self_loops(NextSt,StN,UnwoundTimes,LoopLetter,TrL,NewStN,NewTrL,AccDone).
...
accel_unwound_self_loops(LoopLetter,States,AccelStates) :-
  ...
  do_accel_unwound_self_loops(0,StN,UnwoundTimes,LoopLetter,TrL,NewStN,NewTrL,AccDone),
  ( (AccDone=0, !, AccelStates=States);
    (fsa_regex_compile(min_mh_mb(cleanup(fa(fa(PredModule,NewStN,IniL,FinL,NewTrL,[])))),AccelStates)) ).
...
do_accel_unwound_loops(StN,StN,_,_,_,AugmStN,TrL,AugmStN,TrL,0) :- !.

do_accel_unwound_loops(St,StN,UnwoundTimes,LoopL,LoopLen,AugmStN,TrL,NewStN,NewTrL,1) :-
  successor_via_ntimes_a_word(UnwoundTimes,St,LoopL,TrL,[St],Visited),
  ( (UnwoundTimes=1, !, X=St);
     %% successor_via_word(St,LoopL,TrL,X) ),
     (Visited=[_,X|_]) ),
     %% (Visited=[X|_]) ),
  write('!!! Adding a loop at:'),write(X),write(' !!!'),nl,
  add_accel_unwound_loop(AugmStN,X,LoopL,NewTrL1),
  NewAugmStN is AugmStN + LoopLen - 1,
  NextSt is St+1,
  do_accel_unwound_loops(NextSt,StN,UnwoundTimes,LoopL,LoopLen,NewAugmStN,TrL,NewStN,NewTrL2,_),
  append(NewTrL1,NewTrL2,NewTrL),
  !.

do_accel_unwound_loops(St,StN,UnwoundTimes,LoopL,LoopLen,AugmStN,TrL,NewStN,NewTrL,AccDone) :-
  NextSt is St+1,
  do_accel_unwound_loops(NextSt,StN,UnwoundTimes,LoopL,LoopLen,AugmStN,TrL,NewStN,NewTrL,AccDone).
...
accel_unwound_loops(LoopL,States,AccelStates) :-
  ...
  write('Accelerating via '),write(LoopL),write(' loops unwound '),
    write(UnwoundTimes),write(' times...'),nl,
  do_accel_unwound_loops(0,StN,UnwoundTimes,LoopL,LoopLen,StN,TrL,NewStN,NewTrL,AccDone),
  ( (AccDone=0, !, AccelStates=States);
    (fsa_regex_compile(min_mh_mb(cleanup(fa(fa(PredModule,NewStN,IniL,FinL,NewTrL,[])))),AccelStates)) ).
...
do_accel_several_unwound_loops([LoopL|LoopsRestL],States,AccelStates) :-
  ...
  write('Accelerating via '),write(LoopL),write(' loops unwound '),
    write(UnwoundTimes),write(' times...'),nl,
  #ifndef NewStAccLoop
    do_accel_unwound_loops(0,StN,UnwoundTimes,LoopL,LoopLen,StN,TrL,NewStN,NewTrL,_),
  #else
...
accel_several_unwound_loops(LoopsL,States,MinAccelStates) :-
  ...
  fsa_regex_compile(min_mh_mb(cleanup(fa(AccelStates))),MinAccelStates).
...
septr_do_one_run_ph1(N,FromSt,ToLine,Tr,ITr,TrL,ReachedL,Result,Value) :-
  ...
  #ifndef IndirectTr
    fsa_regex_compile(min_mh_mb(range(compose(fa(FromSt),fa(Tr)))),ToSt),
  #else
    tr(Tr,RealTr),
    fsa_regex_compile(min_mh_mb(range(compose(fa(FromSt),fa(RealTr)))),ToSt),
  #endif
  ToSt=fa(_,NS2,_,_,T2,_),length(T2,NT2),
  #ifdef CompAtmSizes
    stat_update_max(NS2,NT2),
  #endif  
  ...
  ( (ToSt=fa(r(fsa_preds),1,[0],[],[],[]), !, Result=good,Value=ReachedL);  %% No (i.e. empty) successors.
    (sum_bad(Bad),
     ...
     #ifdef CompAtmSizes
       ImmReachBad=fa(_,NS3,_,_,TrL3,_),
       length(TrL3,NT3),
       stat_update_max(NS3,NT3),
     #endif
     #ifdef GenErrReached
       fsa_write_file('error_reached',ImmReachBad),
     #endif
...
septr_do_one_run_ph3(N,FromSt,ToLine,ToSt,Tr,ITr,TrL,ReachedL,Result,Value) :-
  ...
  #else
    ( (member(ToLine,AbsAt), !, abstract(ToSt,ToAbSt));
      (ToAbSt=ToSt) ),
  #endif
  #ifdef CompAtmSizes
    ToAbSt=fa(_,NS1,_,_,TrL1,_),
    length(TrL1,NT1),
    stat_update_max(NS1,NT1),
  #endif 
  sum_bad(Bad),
  write('Intersecting the abstraction with bad states...'),nl,
  fsa_regex_compile(intersect(fa(ToAbSt),fa(Bad)),AbsImmBad),
  #ifdef CompAtmSizes
    AbsImmBad=fa(_,NS2,_,_,TrL2,_),
    length(TrL2,NT2),
    stat_update_max(NS2,NT2),
  #endif
  #ifdef GenErrReached
    fsa_write_file('error_reached',AbsImmBad),
  #endif
  ( (empty(AbsImmBad), !,
...
augment_reachable(Line,[[Line,ReachAtLine]|ReachedL],NewSt,[[Line,ReachAtLine2]|ReachedL]) :-
  fsa_regex_compile(min_mh_mb(union(fa(ReachAtLine),fa(NewSt))),ReachAtLine2),
  #ifdef CompAtmSizes
    ReachAtLine2=fa(_,NS1,_,_,TrL1,_),
    length(TrL1,NT1),
    stat_update_max(NS1,NT1),
  #endif
  !.
...
septr_do_one_run_ph5(N,FromSt,ToLine,ToSt,ToAbSt,Tr,ITr,TrL,ReachedL,[ToErr,ToBackErr],Result,Value) :-
  ...
  write('Computing pre...'),nl,
  #ifndef IndirectTr
    fsa_regex_compile(min_mh_mb(range(compose(fa(ToErr),fa(ITr)))),BackErr),
  #else
    tr(ITr,RealITr),
    fsa_regex_compile(min_mh_mb(range(compose(fa(ToErr),fa(RealITr)))),BackErr),
  #endif
...
septr_do_asmc(IniLine,IniSt,Empty,TrL) :-
  ...
  ( (Result=good, !, write('----- Property holds!'),nl,
     #ifdef CompAtmSizes
       stat_max(NS,NT),
       write('The maximum size of an encountered automaton: '),
       write(NS),
       write('+'),
       write(NT),
       nl,
...
septr_asmc_bad :- 
  ...
  #endif
  #ifdef CompAtmSizes
    IniSt=fa(_,NS1,_,_,TrL1,_),
    length(TrL1,NT1),
    stat_update_max(NS1,NT1),
    bad(Bad),
    Bad=fa(_,NS2,_,_,TrL2,_),
    length(TrL2,NT2),
    stat_update_max(NS2,NT2),
    stat_septr_sizes_of_tr(TrL),
  #endif
  prepare_summary_bad,
  assertz(run(0)),
  septr_do_asmc(IniLine,IniSt,Empty,TrITrL).
...
aut_loop_det(DtSymL,SepSym,States,AccelStates) :-
  unwound_times(UnwTimes),
  States=fa(PredModule,StN,IniL,FinL,TrL,[]),
  aut_loop_det_max_seq(StN,TrL,DtSymL,SepSym,MaxCollSeqLen),
  MaxLoopLen is MaxCollSeqLen//UnwTimes,
  ( (MaxLoopLen>0,
     !,
     do_aut_loop_det(1,MaxLoopLen,UnwTimes,DtSymL,SepSym,StN,TrL,StN,NewStN,NewTrL,0,AccDone),
     ( (AccDone=0, !, AccelStates=States);
       (fsa_regex_compile(min_mh_mb(cleanup(fa(fa(PredModule,NewStN,IniL,FinL,NewTrL,[])))),AccelStates)) ));
    (AccelStates=States) ),
  !.
...
do_aut_loop_det(LoopLen1,MaxLoopLen,UnwTimes,DtSymL,SepSym,StN,TrL,NewStN1,NewStN3,NewTrL3,Acc1,Acc3) :-
  LoopLen1 =< MaxLoopLen,
  !,
  write('Autodetecting loops of length '),write(LoopLen1),
    write(' repeated '),write(UnwTimes),write(' times. '),nl,
    #ifdef NewStAccLoop    
  ...
  do_aut_loop_det(LoopLen2,MaxLoopLen,UnwTimes,DtSymL,SepSym,StN,TrL,NewStN2,NewStN3,NewTrL2,Acc2,Acc3),
  append(NewTrL1,NewTrL2,NewTrL3),
  !.
...
aut_loop_det_iter_states(St,StN,N,TrL,DtSymL,SepSym,NewN1,NewN3,NewTrL3,Acc1,Acc3) :-
  #ifndef StartAutomDetLoopsWithSep
    aut_loop_det_n_stp_via_dt(N,St,TrL,[St],[],TrL,DtSymL,SepSym,St,NewN1,NewN2,NewTrL1,Acc1,Acc2),
  #else
    aut_loop_det_n_stp_via_sep(N,St,TrL,[St],[],TrL,DtSymL,SepSym,St,NewN1,NewN2,NewTrL1,Acc1,Acc2),
  #endif
  NextSt is St + 1,
...
aut_loop_det_n_stp_via_dt(0,St1,_,StViaL1,TrViaL1,TrL,DtSymL,SepSym,St,NewN1,NewN2,NewTrL,Acc1,Acc2) :-
  ...
  unwound_times(UnwTimes),
  UnwTimesMinusOne is UnwTimes-1,
...
#ifdef StartAutomDetLoopsWithSep

aut_loop_det_n_stp_via_sep(0,St1,_,StViaL1,TrViaL1,TrL,DtSymL,SepSym,St,NewN1,NewN2,NewTrL,Acc1,Acc2) :-
  reverse(TrViaL1,TrViaL2),
  %% write('---->>>> '),write(TrViaL2),nl,
  unwound_times(UnwTimes),
  UnwTimesMinusOne is UnwTimes-1,
  aut_loop_det_snd_n_stp(UnwTimesMinusOne,St1,TrL,StViaL1,TrViaL2,TrL,St,St1,[],NewN1,NewN2,NewTrL,Acc1,Acc2),
  !.
...
aut_loop_det_snd_n_stp(1,_,_,_,[],_,_,StB,RefTrViaL1,NewN1,NewN2,NewTrL,_,1) :-
  reverse(RefTrViaL1,RefTrViaL2),
  write('!!! Adding a '),write(RefTrViaL2),write(' loop at: '),write(StB),write(' !!!'),nl,
  add_accel_unwound_loop(NewN1,StB,RefTrViaL2,NewTrL),
  length(RefTrViaL2,LoopLen),
  NewN2 is NewN1 + LoopLen - 1,
  !.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Computing the length of the maximum collapsable path in an automaton of the
% form [sep,dt]*
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Acceleration via automatically detected loops of a certain length
%  -- we consider loops consisting of arbitrary combinations of given data
%     while not giving any special meaning to any separator symbol...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Automatically detecting and adding data loops of a certain length
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

dt_aut_loop_det(DtSymL,States,AccelStates) :-
  unwound_times(UnwTimes),
  States=fa(PredModule,StN,IniL,FinL,TrL,[]),
  dt_aut_loop_det_max_seq(StN,TrL,DtSymL,MaxCollSeqLen),
  MaxLoopLen is MaxCollSeqLen//UnwTimes,
  ( (MaxLoopLen>0,
    %% (MaxLoopLen>MinLoopLen,
     !,
     %% dt_do_aut_loop_det(1,MaxLoopLen,UnwTimes,DtSymL,StN,TrL,StN,NewStN,NewTrL,0,AccDone),
     dt_do_aut_loop_det(2,MaxLoopLen,UnwTimes,DtSymL,StN,TrL,StN,NewStN,NewTrL,0,AccDone),
     %% dt_do_aut_loop_det(MaxLoopLen,MaxLoopLen,UnwTimes,DtSymL,StN,TrL,StN,NewStN,NewTrL,0,AccDone),
     %% dt_do_aut_loop_det(MinLoopLen,MaxLoopLen,UnwTimes,DtSymL,StN,TrL,StN,NewStN,NewTrL,0,AccDone),
     ( (AccDone=0, !, AccelStates=States);
       (fsa_regex_compile(min_mh_mb(cleanup(fa(fa(PredModule,NewStN,IniL,FinL,NewTrL,[])))),AccelStates)) ));
    (AccelStates=States) ),
  !.
...
dt_do_aut_loop_det(LoopLen1,MaxLoopLen,UnwTimes,DtSymL,StN,TrL,NewStN1,NewStN3,NewTrL3,Acc1,Acc3) :-
  LoopLen1 =< MaxLoopLen,
  !,
  write('Autodetecting loops of length '),write(LoopLen1),
    write(' repeated '),write(UnwTimes),write(' times. '),nl,
      ...
      write('  Adding the loops at a new state...'),nl,
    #endif
  dt_aut_loop_det_iter_states(0,StN,LoopLen1,TrL,DtSymL,NewStN1,NewStN2,NewTrL1,Acc1,Acc2),
  LoopLen2 is LoopLen1+1,
  dt_do_aut_loop_det(LoopLen2,MaxLoopLen,UnwTimes,DtSymL,StN,TrL,NewStN2,NewStN3,NewTrL2,Acc2,Acc3),
  append(NewTrL1,NewTrL2,NewTrL3),
  !.
...
dt_do_aut_loop_det(LoopLen1,MaxLoopLen,UnwTimes,DtSymL,StN,TrL,NewStN1,NewStN2,NewTrL2,Acc1,Acc2) :-
  LoopLen1 =< MaxLoopLen,
  !,
  LoopLen2 is LoopLen1+1,
  dt_do_aut_loop_det(LoopLen2,MaxLoopLen,UnwTimes,DtSymL,StN,TrL,NewStN1,NewStN2,NewTrL2,Acc1,Acc2),
  !.

#endif

dt_do_aut_loop_det(_,_,_,_,_,_,NewStN,NewStN,[],Acc,Acc).

%-------------------------------------------------------------------------------

dt_aut_loop_det_iter_states(StN,StN,_,TrL,_,NewN,NewN,TrL,Acc,Acc) :- !.

dt_aut_loop_det_iter_states(St,StN,N,TrL,DtSymL,NewN1,NewN3,NewTrL3,Acc1,Acc3) :-
  dt_aut_loop_det_n_stp(N,St,TrL,[St],[],TrL,DtSymL,St,NewN1,NewN2,NewTrL1,Acc1,Acc2),
  NextSt is St + 1,
  dt_aut_loop_det_iter_states(NextSt,StN,N,TrL,DtSymL,NewN2,NewN3,NewTrL2,Acc2,Acc3),
  append(NewTrL1,NewTrL2,NewTrL3),
  !.

%-------------------------------------------------------------------------------

dt_aut_loop_det_n_stp(N1,St1,TrToDoL1,StViaL1,TrViaL1,TrL,DtSymL,St,NewN1,NewN3,NewTrL3,Acc1,Acc3) :-
  N1>0,
  TrToDoL1=[trans(St1,A,St2)|TrToDoL2],
  not(member(St2,StViaL1)),
  fsa_preds:conjunction(A,in(DtSymL),_),
  N2 is N1-1,
  dt_aut_loop_det_n_stp(N2,St2,TrL,[St2|StViaL1],[A|TrViaL1],TrL,DtSymL,St,NewN1,NewN2,NewTrL1,Acc1,Acc2),
  dt_aut_loop_det_n_stp(N1,St1,TrToDoL2,StViaL1,TrViaL1,TrL,DtSymL,St,NewN2,NewN3,NewTrL2,Acc2,Acc3),
  append(NewTrL1,NewTrL2,NewTrL3),
  !.

dt_aut_loop_det_n_stp(0,St1,_,StViaL1,TrViaL1,TrL,DtSymL,St,NewN1,NewN2,NewTrL,Acc1,Acc2) :-
  reverse(TrViaL1,TrViaL2),
  unwound_times(UnwTimes),
  UnwTimesMinusOne is UnwTimes-1,
  aut_loop_det_snd_n_stp(UnwTimesMinusOne,St1,TrL,StViaL1,TrViaL2,TrL,St,St1,[],NewN1,NewN2,NewTrL,Acc1,Acc2),
...
dt_aut_loop_det_n_stp(_,_,[],_,_,_,_,_,NewN,NewN,[],Acc,Acc) :- !.

dt_aut_loop_det_n_stp(N1,St1,TrToDoL1,StViaL1,TrViaL1,TrL,DtSymL,St,NewN1,NewN2,NewTrL,Acc1,Acc2) :-
  TrToDoL1=[_|TrToDoL2],
  dt_aut_loop_det_n_stp(N1,St1,TrToDoL2,StViaL1,TrViaL1,TrL,DtSymL,St,NewN1,NewN2,NewTrL,Acc1,Acc2),
  !.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Computing the length of the maximum collapsable dt* path in an automaton
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

dt_aut_loop_det_do_comp_max_seq(N1,St1,TrToDoL1,StViaL1,TrL,DtSymL,Max) :-
  TrToDoL1=[trans(St1,A,St2)|TrToDoL2],
  not(member(St2,StViaL1)),
  fsa_preds:conjunction(A,in(DtSymL),_),
  N2 is N1+1,
  dt_aut_loop_det_do_comp_max_seq(N2,St2,TrL,[St2|StViaL1],TrL,DtSymL,Max1),
  dt_aut_loop_det_do_comp_max_seq(N1,St1,TrToDoL2,StViaL1,TrL,DtSymL,Max2),
  ( (Max1 > Max2, !, Max=Max1); (Max=Max2) ),
  !.

dt_aut_loop_det_do_comp_max_seq(N,St,TrToDoL1,StViaL,TrL,DtSymL,Max) :-
  TrToDoL1=[_|TrToDoL2],
  dt_aut_loop_det_do_comp_max_seq(N,St,TrToDoL2,StViaL,TrL,DtSymL,Max),
  !.

dt_aut_loop_det_do_comp_max_seq(N,_,[],_,_,_,N).

%-------------------------------------------------------------------------------

dt_aut_loop_det_comp_max_seq(StN,StN,_,_,0) :- !.

dt_aut_loop_det_comp_max_seq(St1,StN,TrL,DtSymL,Max) :-
  dt_aut_loop_det_do_comp_max_seq(0,St1,TrL,[St1],TrL,DtSymL,Max1),
  St2 is St1+1,
  dt_aut_loop_det_comp_max_seq(St2,StN,TrL,DtSymL,Max2),
  ( (Max1 > Max2, !, Max=Max1); (Max=Max2) ),
  !.

%-------------------------------------------------------------------------------

dt_aut_loop_det_max_seq(StN,TrL,DtSymL,Max) :- 
  dt_aut_loop_det_comp_max_seq(0,StN,TrL,DtSymL,Max),
  write('@@@ Current max coll. seq. length: '),write(Max),nl.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Statistics about the sizes of automata...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

stat_max(NS1,NT1,NS2,NT2,NS,NT) :- 
  N1 is NS1+NT1,
  N2 is NS2+NT2,
  ( (N1>N2, !, NS=NS1, NT=NT1); 
    (NS=NS2, NT=NT2) ),
...
stat_septr_do_comp_sizes_of_tr([[_,A]|L],NS,NT) :-
  A=fa(_,NS1,_,_,TrL1,_),
  length(TrL1,NT1),
  stat_septr_do_comp_sizes_of_tr(L,NS2,NT2),
  stat_max(NS1,NT1,NS2,NT2,NS,NT),
  !.
  
stat_septr_do_comp_sizes_of_tr([],0,0).

%...............................................................................

stat_septr_comp_sizes_of_tr([[_,AL]|L],NS,NT) :-
  stat_septr_do_comp_sizes_of_tr(AL,NS1,NT1),
  stat_septr_comp_sizes_of_tr(L,NS2,NT2),
  stat_max(NS1,NT1,NS2,NT2,NS,NT),
  !.
  
stat_septr_comp_sizes_of_tr([],0,0).

%...............................................................................

stat_septr_sizes_of_tr(TrL) :-
  stat_septr_comp_sizes_of_tr(TrL,NS,NT),
  stat_update_max(NS,NT).

%-------------------------------------------------------------------------------

stat_update_max(NS1,NT1) :-
  ( (retract(stat_max(NS2,NT2)), !); 
    (NS2=0,NT2=0) ),
  stat_max(NS1,NT1,NS2,NT2,NS,NT), 
  assertz(stat_max(NS,NT)).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Transforming a transducer to a length-preserving one as in HV-Infinity 2004
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

take_not_in([],_,[]) :- !.

take_not_in([A|L1],L2,L3) :-
  member(A,L2),
  take_not_in(L1,L2,L3),
  !.
  
take_not_in([A|L1],L2,[A|L3]) :-
  take_not_in(L1,L2,L3),
  !.
  
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

transd_do_remove_notin(trans(F,not_in(A) / not_in(B),T),Sigma,trans(F,in(C) / in(D),T)) :- 
  take_not_in(Sigma,A,C),
  take_not_in(Sigma,B,D),
  !.

%----------------------------------------------------------------------------------

transd_do_remove_notin(trans(F,in(A) / not_in(B),T),Sigma,trans(F,in(A) / in(C),T)) :- 
  take_not_in(Sigma,B,C),
  !.

%----------------------------------------------------------------------------------

transd_do_remove_notin(trans(F,not_in(A) / in(B),T),Sigma,trans(F,in(C) / in(B),T)) :- 
  take_not_in(Sigma,A,C),
  !.

%----------------------------------------------------------------------------------

transd_do_remove_notin(trans(F,$@(not_in(A)) / $@(not_in(A)),T),Sigma,trans(F,$@(in(B)) / $@(in(B)),T)) :- 
  take_not_in(Sigma,A,B),
  !.

%----------------------------------------------------------------------------------

transd_do_remove_notin(trans(F,not_in(A) / B,T),Sigma,trans(F,in(C) / B,T)) :- 
  take_not_in(Sigma,A,C),
  !.

%----------------------------------------------------------------------------------

transd_do_remove_notin(trans(F,A / not_in(B),T),Sigma,trans(F,A / in(C),T)) :-
  take_not_in(Sigma,B,C),
  !.

%----------------------------------------------------------------------------------

transd_do_remove_notin(trans(F,A / B,T),Sigma,trans(F,A / B,T)) :- !.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

transd_remove_notin([T1|L1],Sigma,[T2|L2]) :-
  transd_do_remove_notin(T1,Sigma,T2),
  transd_remove_notin(L1,Sigma,L2),
  !.

transd_remove_notin([],_,[]) :- !.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

transd_remove_epsilon_trs([trans(F,[] / [],T)|L1],[trans(F,'_b' / '_b',T)|L2]) :- 
  transd_remove_epsilon_trs(L1,L2),
  !.

transd_remove_epsilon_trs([trans(F,[] / B,T)|L1],[trans(F,'_b' / B,T)|L2]) :- 
  transd_remove_epsilon_trs(L1,L2),
  !.

transd_remove_epsilon_trs([trans(F,A/ [],T)|L1],[trans(F,A/ '_b',T)|L2]) :-
  transd_remove_epsilon_trs(L1,L2),
  !.

transd_remove_epsilon_trs([trans(F,A / B,T)|L1],[trans(F,A / B,T)|L2]) :-
  transd_remove_epsilon_trs(L1,L2),
  !.

transd_remove_epsilon_trs([],[]) :- !.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

add_blankblank_loops(N,N,[]) :- !.

add_blankblank_loops(M,N,[trans(M,'_b' / '_b',M)|L]) :- 
  MM is M+1,
  add_blankblank_loops(MM,N,L),
  !.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

do_transd_to_lenpr(Transd1,Sigma,Transd2) :- 
  Transd1 = fa(t(fsa_preds,fsa_preds),NQ,IniL,FinL,TrL1,[]),
  transd_remove_notin(TrL1,Sigma,TrL2),
  transd_remove_epsilon_trs(TrL2,TrL3),
  add_blankblank_loops(0,NQ,BBLoops),
  append(TrL3,BBLoops,TrL4),
  sort(TrL4,TrL5),
  fsa_regex_compile(min_mh_mb(fa(fa(t(fsa_preds,fsa_preds),NQ,IniL,FinL,TrL5,[]))),Transd2),
  !.

%----------------------------------------------------------------------------------

transd_to_lenpr(File1,Sigma1,File2) :-
  sort(Sigma1,Sigma2),
  fsa_regex_compile(file(File1),Transd1),
  do_transd_to_lenpr(Transd1,Sigma2,Transd2),
  fsa_write_file(File2,Transd2),
  !.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Transforming an automaton to a length-preserving one as in HV-Infinity 2004
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

atm_do_remove_notin(trans(F,not_in(A),T),Sigma,trans(F,in(B),T)) :- 
  take_not_in(Sigma,A,B),
  !.

%----------------------------------------------------------------------------------

atm_do_remove_notin(trans(F,A,T),Sigma,trans(F,A,T)) :- !.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

atm_remove_notin([T1|L1],Sigma,[T2|L2]) :-
  atm_do_remove_notin(T1,Sigma,T2),
  atm_remove_notin(L1,Sigma,L2),
  !.
...
add_blank_loops(M,N,[trans(M,'_b',M)|L]) :- 
  MM is M+1,
  add_blank_loops(MM,N,L),
  !.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

do_atm_to_lenpr(Atm1,Sigma,Atm2) :-
  Atm1 = fa(r(fsa_preds),NQ,IniL,FinL,TrL1,[]),
  atm_remove_notin(TrL1,Sigma,TrL2),
  add_blank_loops(0,NQ,BLoops),
  append(TrL2,BLoops,TrL3),
  sort(TrL3,TrL4),
  fsa_regex_compile(min_mh_mb(fa(fa(r(fsa_preds),NQ,IniL,FinL,TrL4,[]))),Atm2),
  !.

%----------------------------------------------------------------------------------

atm_to_lenpr(File1,Sigma1,File2) :-
  sort(Sigma1,Sigma2),
  fsa_regex_compile(file(File1),Atm1),
  do_atm_to_lenpr(Atm1,Sigma2,Atm2),
  fsa_write_file(File2,Atm2),
  !.
