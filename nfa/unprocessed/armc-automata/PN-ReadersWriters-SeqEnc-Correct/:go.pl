...
abstr_fw(Conc,Abstr) :-
  ...
  fsa_construct_rename_states(ConcSym,NewConcIniL,NewConcFinL,NewConcTrL,[],Abstr1),
    save_nfa(Abstr1,afterFwPredAbs),
...
abstr_bw(Conc,Abstr) :-
  ...
  adjust_st(PartitionL,ConcIniL,NewConcIniL),
  fsa_construct_rename_states(ConcSym,NewConcIniL,NewConcFinL,NewConcTrL,[],Abstr1),
    save_nfa(Abstr1,afterBwPredAbs),
  fsa_regex_compile(mh(fa(Abstr1)),Abstr).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Saving non-determinized FA in the FSA format
...
save_trans([Tr|_]) :-
  ...
  halt.
%...............................................................................
% Saving non-determinized FA in the Timbuk format
% (Str is a string to put into the file name.)
...
do_one_run(N,AbsReach,Result,Value) :-
  ...
  write('Abstract source automaton: '),write(NS1),write(','),write(NT1),nl,
      fsa_regex_compile(range(compose(fa(AbsReach),fa(TR))),NewReachNFA),
      save_nfa(NewReachNFA,afterTR),
      fsa_regex_compile(mh(fa(NewReachNFA)),NewReach),
...
do_one_run_ph2(N,AbsReach,NewReach,Result,Value) :-
  ...
  fsa_regex_compile(intersect(fa(NewAbsReach),fa(Bad)),AbsImmBad),
    save_nfa(AbsImmBad,intrsAbsBad),
  ( (empty(AbsImmBad), !,
  ...
  ( (SubResult=good, !,Result=SubResult,Value=SubValue);
    ...
    (itr(ITR),
     ...
     write('Computing pre...'),nl,
         fsa_regex_compile(range(compose(fa(NextReachBad1),fa(ITR))),SrcBad1),
         save_nfa(SrcBad1,afterITR),
         fsa_regex_compile(mh(fa(SrcBad1)),SrcBad),
...
accel_unwound_self_loops(LoopLetter,States,AccelStates) :-
  ...
  ( (AccDone=0, !, AccelStates=States);
      (fsa_regex_compile(cleanup(fa(fa(PredModule,StN,IniL,FinL,NewTrL,[]))),AccelStatesAux),
       save_nfa(AccelStatesAux,unfLoopAbs),
       fsa_regex_compile(mh(fa(AccelStatesAux)),AccelStates)) ).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The main loop of accelerating via unwound self-loops
...
accel_unwound_loops(LoopL,States,AccelStates) :-
  ...
  ( (AccDone=0, !, AccelStates=States);
      (fsa_regex_compile(cleanup(fa(fa(PredModule,NewStN,IniL,FinL,NewTrL,[]))),AccelStatesAux),
       save_nfa(AccelStatesAux,unfLoopAbs),
       fsa_regex_compile(mh(fa(AccelStatesAux)),AccelStates)) ).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The main loop of accelerating via unwound general loops
...
accel_several_unwound_loops(LoopsL,States,MinAccelStates) :-
  do_accel_several_unwound_loops(LoopsL,States,AccelStates),
    fsa_regex_compile(cleanup(fa(AccelStates)),MinAccelStatesAux),
    save_nfa(MinAccelStatesAux,unfLoopAbs),
    fsa_regex_compile(mh(fa(MinAccelStatesAux)),MinAccelStates).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
